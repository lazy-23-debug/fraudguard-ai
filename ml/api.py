...
# ── Rebuild the exact same architecture from train.py ────────────────────────
# Loading weights only (not the full .keras file) makes this version-agnostic.
# Works with any Keras/TF version — weights are just numpy arrays in HDF5.
...
def _load_models() -> bool:
    ...
    try:
        ...
        for p in [weights_path, xgb_path, meta_path]:
        ...
        print("[API] Loading Autoencoder weights...")
...
def _recommendation(level: str, amount: float, tx_type: str) -> str:
    ...
    msgs = {
        ...
        'CRITICAL': f"🚨 CRITICAL — Block Immediately. Freeze the {a} {t} transaction and initiate account lockout. File STR with NIBSS."
    }
